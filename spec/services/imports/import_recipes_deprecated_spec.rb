...
RSpec.describe Imports::ImportRecipesDeprecated do
  ...
  context 'when a recipe already exists with the same name' do
    ...
    it 'logs an information' do
      import_recipes

      expect(Rails.logger).to have_received(:info).with(
        'Golden Sweet Cornbread already exists in database... skipping recipe'
      )
    end
  end
  ...
  context 'when an exception is raised' do
    ...
    it 'logs an information' do
      ...
      expect(Rails.logger).to have_received(:info).with(
        "ERROR encountered (Validation failed: Title can't be blank)... skipping recipe"
      )
    end
  end
end
